...
while check:
    ...
    if len(enter_poly) > 0:
        enter_poly_np = np.array(enter_poly, np.int32).reshape((-1, 1, 2))  # Ensure correct shape for fillPoly
        cv.fillPoly(overlay, [enter_poly_np], yellow)
        alpha = 0.5  # Transparency level
...
while True:
    try:
        ...
        for x in result[0].boxes:
            ...
            enter_inter_area = polygon_intersection_area(enter_poly, pix_polygon)
            cv.fillPoly(overlay, [np.array(enter_poly, np.int32).reshape((-1, 1, 2))], yellow)

            if park_area > 0:
                enter_overlap_percentage = (inter_area / park_area) * 100

            if enter_overlap_percentage >20:
                cv.fillPoly(overlay, [np.array(enter_poly, np.int32).reshape((-1, 1, 2))], yellow)
