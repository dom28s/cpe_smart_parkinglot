...
def load_park_from_json(filename):
    ...
    if os.path.exists(filename):
        ...
        if filename == 'enter.json':
            with open(filename, 'r') as f:
                enter_data_load = json.load(f)
                enter_data = [np.array(polygon, np.int32) for polygon in enter_data_load]  # Convert to NumPy array

def save_park_to_json(filename):
    global park_data, enter_data
    if filename == 'enter.json':
        with open(filename, 'w') as f:
            json.dump(enter_data[0], f)
    if filename == 'park.json':
        with open(filename, 'w') as f:
...
def draw_enter(event, x, y, flags, param):
    ...
    if event == cv.EVENT_LBUTTONDOWN:
        ...
        if len(points) == 4:
            points.append(points[0])  # Close the polygon
            enter_data.append(points.copy())  # Convert to NumPy array
            print(enter_data)
...
while check:
    ...
    if len(enter_data) == 5: 
        ...
        for shape in enter_data:
            points_array = shape.reshape((-1, 1, 2))  
            cv.fillPoly(overlay, [enter_], yellow)
        alpha = 0.5  
